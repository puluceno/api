 ...
 * 
 */
public class AdminRFNewRestaurantNotificator extends Notificator {
     ...
     */
    public AdminRFNewRestaurantNotificator() {
	setTemplate("mailtemplates/adminnewrestaurant.mailtemplate");
	...
	String conteudo = MessageFormat.format(template, emailData.get("subsidiaryUrl"), emailData.get("logo"),
		emailData.get("subsidiaryName"), emailData.get("facebook"), emailData.get("restaurantName"),
		emailData.get("cnpj"), emailData.get("subsidiaryCity"), emailData.get("subsidiaryNeighborhood"),
		emailData.get("idSubsidiary"));
	
	// cria a mensagem
	MessageNotificator notificacao = new MessageNotificator("suporte@redefood.com.br");
	
	//	Collection<String> destinatarios = new ArrayList<String>();
	//	destinatarios.add("suporte@redefood.com.br");
	//	destinatarios.add("puluceno@gmail.com");
	//	destinatarios.add("rafaelmotta021@gmail.com");
	//	notificacao.setDestinatarios(destinatarios);
	notificacao.setAssunto("[RedeFood] - Cadastro de Estabelecimento - Ação necessária!");
