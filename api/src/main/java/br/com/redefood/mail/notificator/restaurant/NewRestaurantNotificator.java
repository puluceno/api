 ...
 * 
 */
public class NewRestaurantNotificator extends Notificator {
     ...
     */
    public NewRestaurantNotificator() {
	setTemplate("mailtemplates/newrestaurant.mailtemplate");
	...
	String conteudo = MessageFormat.format(template, emailData.get("subsidiaryUrl"), emailData.get("logo"),
		...
		emailData.get("restaurantName"), emailData.get("urlHelp"), emailData.get("contactEmail"),
		emailData.get("footerSlogan"));
	
	// cria a mensagem
	MessageNotificator notificacao = new MessageNotificator(emailData.get("addressee"));
	notificacao.setAssunto("[RedeFood] - Cadastro de Estabelecimento");
