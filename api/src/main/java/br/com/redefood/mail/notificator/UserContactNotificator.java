 ...
 * 
 */
public class UserContactNotificator extends Notificator {
     ...
     */
    public UserContactNotificator() {
	setTemplate("mailtemplates/usercontact.mailtemplate");
	...
	String conteudo = MessageFormat.format(template, emailData.get("subsidiaryUrl"), emailData.get("logo"),
		...
		emailData.get("userEmail"),emailData.get("message"), emailData.get("urlHelp"), emailData.get("contactEmail"),
		emailData.get("footerSlogan"));
	
	// cria a mensagem
	MessageNotificator notificacao = new MessageNotificator(emailData.get("addressee"));
	notificacao.setAssunto("[RedeFood] - Contato de usuário");
