 ...
 * 
 */
public class NewRestaurantSuccessNotificator extends Notificator {
	 ...
	 */
	public NewRestaurantSuccessNotificator() {
		setTemplate("mailtemplates/newrestaurantsuccess.mailtemplate");
	...
	protected MessageNotificator prepareMessage(final String template, final HashMap<String, String> emailData)
		...
		String conteudo = MessageFormat.format(template, emailData.get("subsidiaryUrl"), emailData.get("logo"),
				...
				emailData.get("restaurantName"), emailData.get("password"), emailData.get("cpf"), emailData.get("adminUrl"));

		// cria a mensagem
		MessageNotificator notificacao = new MessageNotificator(emailData.get("addressee"));
		notificacao.setAssunto("[RedeFood] - Estabelecimento cadastrado com sucesso!");
		notificacao.setConteudo(conteudo);

		return notificacao;
	} // fim do método prepareMensagem(String, HashMap<String, String>)

} // fim da classe NewRestaurantSuccessNotificator
